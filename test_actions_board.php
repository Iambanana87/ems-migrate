...
$endpoints = [
    [
        'name' => 'actions_board (active, all)',
        'legacy' => "http://localhost:8080/ems/api.php?action=actions_board&tab=active&process=all",
        'laravel' => "http://localhost:8000/api/gateway?c=Report&m=actionsBoard&tab=active&process=all"
    ],
    [
        'name' => 'actions_board (done, mold)',
        'legacy' => "http://localhost:8080/ems/api.php?action=actions_board&tab=done&process=mold",
        'laravel' => "http://localhost:8000/api/gateway?c=Report&m=actionsBoard&tab=done&process=mold"
    ],
    [
        'name' => 'actions_board (invalid tab and process - checks fallback)',
        'legacy' => "http://localhost:8080/ems/api.php?action=actions_board&tab=invalid&process=invalid",
        'laravel' => "http://localhost:8000/api/gateway?c=Report&m=actionsBoard&tab=invalid&process=invalid"
    ]
];
...
foreach ($endpoints as $ep) {
    ...
    if ($legacyJson === $laravelJson) {
        ...
        file_put_contents(__DIR__ . '/legacy_actions_out.json', $legacyJson);
        file_put_contents(__DIR__ . '/laravel_actions_out.json', $laravelJson);
        echo "Saved to legacy_actions_out.json and laravel_actions_out.json\n";
        $allPassed = false;
        
        file_put_contents(__DIR__ . '/json_diff_actions.php', '<?php
